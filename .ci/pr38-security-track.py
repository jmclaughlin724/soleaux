#!/usr/bin/env python3
"""Apply the PR #38 runtime-admission and revalidation fixes."""
...
if count != 1:
    raise SystemExit(f"remove ProbeEvidence: expected one match, found {count}")
...
if count != 1:
    raise SystemExit(f"remove caller-owned canonical proof: expected one match, found {count}")

start = compatibility.index("    if !version.mutation_eligible {")
end = compatibility.index("\n#[allow(clippy::too_many_arguments)]", start)
replacement = r"""    let passed_signals = capabilities
        .get("soleauxProbe")
...
"""
compatibility = compatibility[:start] + replacement + compatibility[end:]

compatibility = replace_once(
    compatibility,
    "    let mut write_eligible = 0usize;\n",
    "",
    "remove external write-eligible counter",
)
compatibility = replace_once(
    compatibility,
    r"""            if version.mutation_eligible {
                write_eligible += 1;
...
""",
    "deny external mutation eligibility",
)
compatibility = replace_once(
    compatibility,
    r"""    if write_eligible != 1 {
        bail!("exactly one capability matrix entry must be mutation eligible");
    }
""",
    "",
    "remove one-write-entry requirement",
)
...
bind_marker = r"""    validate_json_field(&capabilities, "binding capabilities")?;
    validate_json_field(&metadata, "binding metadata")?;
    let now = unix_ms();
"""
bind_replacement = r"""    validate_json_field(&capabilities, "binding capabilities")?;
    validate_json_field(&metadata, "binding metadata")?;
...
helper = r"""fn revalidate_client(
    ...
    let changed = touch_last_seen
        || ttl_ms.is_some()
        || payload.capabilities != existing.payload.capabilities
...
registry = replace_once(
    ...
    "insert client revalidation helper",
)
...
mod compatibility_regression_tests {
    ...
    fn bind_revalidates_and_downgrades_a_stale_external_client() {
        ...
        assert_eq!(
            read_only["binding"]["payload"]["accessMode"],
            "read_only"
        );
    }
}
"""
if "#[cfg(test)]\nmod compatibility_regression_tests" not in registry:
    registry = registry.rstrip() + "\n" + test_module + "\n"
write("native/daemon/ipc/src/registry.rs", registry)

# Security-track tests need the local matrix to match the new fail-closed rule.
