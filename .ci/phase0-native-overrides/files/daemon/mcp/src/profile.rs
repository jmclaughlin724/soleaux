//! Binding Phase 0 public MCP and context-packet contract.
//!
//! The runtime remains transitional until Phase 1 implements the complete
//! twelve-tool surface. This module locks names, ordering, ceiling, version,
//! digests, and the native-selection requirements at compile time.
...
mod tests {
    ...
    fn optional_candidates_are_unique_and_do_not_expand_the_ceiling() {
        ...
        assert!(
            OPTIONAL_TOOL_NAMES
                .iter()
                .all(|name| !CANONICAL_TOOL_NAMES.contains(name))
        );
    }

    #[test]
    fn embedded_contracts_lock_version_digests_and_native_guarantees() {
        let profile: Value = serde_json::from_str(PROFILE_MANIFEST_JSON).expect("profile JSON");
        let context: Value =
