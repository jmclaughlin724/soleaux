...
pub fn redact_sensitive_text(source: &str) -> (String, usize) {
    redact_sensitive_source(source)
}
...
fn redact_sensitive_source(source: &str) -> (String, usize) {
    ...
    for line in source.split_inclusive('\n') {
        ...
        let (assignment_redacted, assignment_count) = redact_sensitive_assignment(body);
        let (prefixed_redacted, prefix_count) = redact_prefixed_tokens(&assignment_redacted);
        count = count
            .saturating_add(assignment_count)
            .saturating_add(prefix_count);
        output.push_str(&prefixed_redacted);
        if newline {
            output.push('\n');
        }
    }
...
fn redact_sensitive_assignment(line: &str) -> (String, usize) {
    ...
    // Preserve a trailing comma for object/config snippets while discarding the
    // secret value and any inline comment that could repeat it.
    if right.ends_with(',') {
        rendered.push(',');
    }
    if rendered.trim().is_empty() {
        rendered = format!("{}[REDACTED]", " ".repeat(indentation));
    }
    (rendered, 1)
}
...
fn redact_prefixed_tokens(line: &str) -> (String, usize) {
    ...
    // Authorization headers are common in source fixtures but do not have a
    // stable vendor prefix.
    let mut cursor = 0usize;
    while let Some(relative) = output[cursor..].find("Bearer ") {
