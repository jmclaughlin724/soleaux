...
def create_parser() -> argparse.ArgumentParser:
    ...
    attach = subparsers.add_parser(
        "attach",
        help="Attach a consumer repo: host MCP registrations, soleaux.toml, deployment.",
    )
    attach.add_argument(
        "--repo",
        type=pathlib.Path,
        help="Consumer repository root (default: --root or the current directory).",
    )
    attach.add_argument(
        "--command",
        dest="launcher",
        default="uvx",
        help="Launcher command written into host registrations (default: uvx).",
    )
    attach.add_argument(
        "--shared",
        action="store_true",
        help="Record intent for the machine-registry shared service (Stage 4).",
    )
    attach.add_argument("--dry-run", action="store_true", help="Print the plan without writing.")
    attach.add_argument("--yes", action="store_true", help="Skip the interactive confirmation.")
...
async def run_cli(
    ...
    *,
    ...
    if args.command == "attach":
        return _run_attach(args, root, stdout=output, stderr=sys.stderr)
    if args.command == "mcp":
...
def _generate_soleaux_toml(root: pathlib.Path, output: pathlib.Path, stdout: typing.TextIO) -> int:
    ...
    content = render_soleaux_toml(root)
...
def _run_attach(
    ...
    *,
    ...
    """Run the attach workflow and close with the validation route."""
    from soleaux.provisioning import attach as attach_mod
    from soleaux.provisioning.contracts import AdoptExtraMissingError

    target_root = (args.repo or root).resolve()
    options = attach_mod.AttachOptions(command=args.launcher, shared=bool(args.shared))
    try:
        plan = attach_mod.build_plan(target_root, options=options)
        stdout.write(attach_mod.render_plan(plan))
        stdout.write("\n")
        for warning in plan.warnings:
            stderr.write(f"warning: {warning}\n")

        if not plan.actions or args.dry_run:
            return 0
        ...
        result = attach_mod.apply_plan(plan, options=options)
        for entry in result.written:
            stdout.write(f"wrote: {entry}\n")
        for entry in result.skipped:
            stdout.write(f"skipped (already in desired state): {entry}\n")
        ...
        exit_code = _check_mcp(target_root, json_output=False, stdout=stdout)
        stdout.write(
            "\nAttached. Validate with: "
            f"soleaux --root {target_root} check mcp\n"
            "Serve with: soleaux bridge --context codex (objective on stdin)\n"
        )
        return exit_code
    except AdoptExtraMissingError as exc:
        stderr.write(f"{exc}\n")
