...
Apply only the steps needed for the affected interaction. A small state change does not require a route-wide transition system.

## Step 1: Audit the App

Before writing code, inspect the affected navigation or state-change flow. Search that scope for:

- **Relevant `<Link>` and programmatic navigation calls** — these are the triggers for the selected flow.
- **Relevant `<Suspense>` boundaries** — check what their fallback and reveal render.
- **Participating route components** — decide placement only for the source and destination in scope.
...
## Step 2: Select CSS Recipes

Copy only the recipes used by the selected interaction plus their reduced-motion override. Keep existing product timing and motion tokens when they exist. Treat `css-recipes.md` as examples to adapt, not a second global design-system owner.

## Step 3: Isolate Persistent Elements

For a persistent element that visibly participates in the selected transition, add a unique `viewTransitionName` to separate it from the page snapshot:
...
If the same verified transition contract repeats across real routes, extract a small wrapper:
...
Keep the wrapper local until multiple consumers share the same behavior.
...
- Start with page-level placement. A persistent layout usually receives updates rather than route mount/unmount enter/exit; use a layout boundary only for an intentional nested effect that has been tested with its child boundaries.
...
This example uses `slide-down` / `slide-up` when the loading handoff has a real vertical relationship. For a simpler replacement, use a cross-fade. Choose the narrowest motion that communicates the change and preserve the reduced-motion path from `css-recipes.md`.
...
When list items contain shared elements, compose list identity and shared-element identity with two nested layers; see [Composing Shared Elements with List Identity](patterns.md#composing-shared-elements-with-list-identity).
...
- **Browser-history back navigation** — it does not carry a transition type, so a type-keyed directional slide will not run. Shared-element morphs can still run. Use an explicit link or `router.push` with `transitionTypes` only when the product requires directional back motion.
