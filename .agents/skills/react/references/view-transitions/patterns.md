...
Use one keyed layer for list insertion, removal, or reordering and a nested named layer for the element that morphs into another view. The two identities solve different problems and should not share one transition contract:
...
Keep both names stable and globally unique. If list filtering should not animate, remove the outer enter/exit layer and retain only the named shared element.
...
**Browser back/forward has no transition type:** Type-keyed directional slides do not run, but matching named elements can still morph. Use an explicit link or `router.push(..., { transitionTypes })` only when directional history motion is required.
...
**Layout and page effects compete:** A persistent layout boundary usually observes an update while the page boundary owns mount/unmount. Nested transitions are valid, but their responsibilities and names must not compete. Temporarily remove the layout effect to isolate the issue, then verify the intended nesting against the real navigation and Suspense paths.
