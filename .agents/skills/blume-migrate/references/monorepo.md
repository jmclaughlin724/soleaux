...
pnpm 11 defaults its supply-chain guard, `minimumReleaseAge`, to `1440` minutes (24 hours); earlier majors default to `0` unless the policy is configured. The pnpm 11 default can be active even when the repository has no `minimumReleaseAge` key and `pnpm config get minimumReleaseAge` reports `undefined`. A newly published selected Blume version can therefore fail an exact install or lockfile verification, while a range can resolve an older eligible version.

**Do not disable the policy globally** — that removes the guard for the whole dependency tree. When immediate installation is required, exempt only the exact selected Blume version. A bare `blume` entry would exempt every future release too.

**Resolve the effective policy and publication time:**

```bash
pnpm --version
pnpm config get minimumReleaseAge
pnpm config get minimumReleaseAgeExclude
pnpm view "blume@<selected-version>" time --json
```

Interpret an undefined age using the pnpm major: pnpm 11 still applies its built-in 1440-minute default, while earlier majors default to 0. An explicit value wins. Compare the selected version's publication timestamp with the effective window before adding an exception. Exact-version exclusions require pnpm 10.19 or newer; on an older pnpm, wait for the version to mature or upgrade pnpm instead of adding a package-wide exemption.

**Fix in `pnpm-workspace.yaml`** by preserving existing entries and adding one exact, temporary selector:

```yaml
minimumReleaseAgeExclude:
  - "blume@<selected-version>"
```

Preserve any explicitly configured `minimumReleaseAge`; do not add or change it merely to make the exclusion work. pnpm 11 reads project-level non-registry settings from `pnpm-workspace.yaml`, not `.npmrc`. Remove the exact exclusion after the version matures so later Blume releases remain protected.

Report: "Added the selected `blume@<version>` to `minimumReleaseAgeExclude` so it can install before the window expires; the release-age guard still applies to every other version and package."
...
- [ ] Resolved the effective `minimumReleaseAge`, including pnpm 11's built-in default; if the selected release is still immature, added only its exact `blume@<version>` selector without changing the guard.
