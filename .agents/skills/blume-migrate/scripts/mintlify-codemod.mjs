...
const MARKDOWN_FILE_PATTERN = /\.mdx?$/u;
...
const findUnquotedKeySeparator = (
  line,
  start,
  requireWhitespaceAfterSeparator
) => {
...
const parseKeyLine = (line, start, requireWhitespaceAfterSeparator) => {
  const firstCharacter = line[start];
  if (
    firstCharacter === undefined ||
    isWhitespace(firstCharacter) ||
    firstCharacter === ":"
  ) {
    return null;
  }
  ...
  if (!isQuote(firstCharacter)) {
    const separator = findUnquotedKeySeparator(
      line,
      start,
      requireWhitespaceAfterSeparator
    );
    ...
    return {
      key: line.slice(start, separator).trimEnd(),
      separator,
    };
  }

  let closingQuote = start + 1;
  while (closingQuote < line.length && !isQuote(line[closingQuote])) {
    closingQuote += 1;
  }
  const key = line.slice(start + 1, closingQuote);
  if (
    line[closingQuote] !== firstCharacter ||
    key === "" ||
    isWhitespace(key[0]) ||
    key[0] === ":"
  ) {
  ...
  let separator = closingQuote + 1;
  while (isWhitespace(line[separator])) {
    separator += 1;
  }
  const nextCharacter = line[separator + 1];
  if (
    line[separator] !== ":" ||
    (requireWhitespaceAfterSeparator &&
      nextCharacter !== undefined &&
...
const parseNestedKey = (line) => {
  ...
  return parseKeyLine(line, start, false)?.key ?? null;
};

const parseIconLine = (line) => {
  let start = 0;
  while (isWhitespace(line[start])) {
    start += 1;
  }
  const indent = line.slice(0, start);
  if (!line.startsWith("icon:", start)) {
    return null;
  }

  let cursor = start + "icon:".length;
  while (isWhitespace(line[cursor])) {
    cursor += 1;
  }
  ...
  if (isQuote(quote)) {
    ...
    if (cursor === line.length) {
      return { comment: null, indent, name };
    }
    if (line[cursor] !== "#") {
      return null;
    }
    return { comment: line.slice(cursor), indent, name };
  }
  ...
  return {
    comment: line[cursor] === "#" ? line.slice(cursor) : null,
    indent,
    name: line.slice(nameStart, cursor).trimEnd(),
  };
};
...
const topKey = (line) => {
  const parsed = parseKeyLine(line, 0, true);
  if (!parsed) {
    return null;
  }
  return {
    key: parsed.key,
    value: line.slice(parsed.separator + 1).trim(),
  };
};
...
const blockRange = (fm, start) => {
  let end = start + 1;
  while (end < fm.length && (fm[end] === "" || isWhitespace(fm[end][0]))) {
    end += 1;
  }
...
const findChild = (fm, start, endExclusive, childKey) => {
  for (let index = start + 1; index < endExclusive; index += 1) {
    if (parseNestedKey(fm[index]) === childKey) {
      return index;
    }
  }
  return -1;
...
const remapIcons = (fm) => {
  ...
  while (index < fm.length) {
    const parsed = parseIconLine(fm[index]);
    if (!parsed) {
      index += 1;
      continue;
    }
    ...
    if (lucide !== raw) {
      ...
      fm[index] = `${parsed.indent}icon: ${lucide}${comment}`;
...
const setNested = (fm, parent, child, value) => {
  for (let index_ = 0; index_ < fm.length; index_ += 1) {
    const tk = topKey(fm[index_]);
    if (!tk || tk.key !== parent) {
      continue;
    }
    if (tk.value !== "") {
    ...
    const [start, end] = blockRange(fm, index_);
...
const rewriteFields = (fm) => {
  ...
  for (let index = fm.length - 1; index >= 0; index -= 1) {
    const tk = topKey(fm[index]);
    if (!tk) {
      continue;
    }
    if (DROP.has(tk.key)) {
      const [start, end] = blockRange(fm, index);
...
const transform = (text) => {
  ...
  const isChanged =
    rebuilt !== text && changes.some((c) => MUTATING.has(c.kind));
  return { changed: isChanged, changes, text: isChanged ? rebuilt : text };
};
...
const collectFiles = (paths) => {
  ...
  const walk = (p) => {
    const st = statSync(p);
    if (st.isDirectory()) {
      for (const name of readdirSync(p).toSorted(compareStrings)) {
        if (name === "node_modules" || name.startsWith(".")) {
          continue;
        }
        walk(join(p, name));
      }
    } else if (MARKDOWN_FILE_PATTERN.test(p)) {
      out.push(p);
    }
  };
  for (const p of paths.toSorted(compareStrings)) {
    walk(p);
  }
  return out;
};
...
const main = () => {
  ...
  for (const file of files) {
    ...
    report.push({ changes, file: relative(process.cwd(), file) });
  }
