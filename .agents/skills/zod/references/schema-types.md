...
```typescript
// Direct literal tuple
z.enum(["active", "inactive", "pending"]);

// Variable tuple: use as const so Zod infers literal values
const statuses = ["active", "inactive", "pending"] as const;
z.enum(statuses);

// Repo-owned enum-like object: prefer this over TypeScript enum declarations
const Status = {
  Active: "active",
  Inactive: "inactive",
} as const;
...
// Numeric enum-like object, only when runtime inputs are numbers
const Priority = {
  Low: 0,
  Medium: 1,
  High: 2,
...
// Existing external or legacy TypeScript enums can be bridged
z.enum(ExternalStatus);
```
