...
1. Use `~/.codex/config.toml` for personal machine settings: auth, provider endpoints, telemetry, notification, profiles, local preferences, and personal `developer_instructions`.
...
- Keep `developer_instructions` compact and stable. Put dynamic task paths, current plan state, examples, and session findings in user or task context.
...
- For skill edits, run `pnpm skills:audit`. For Codex rule edits, run `pnpm execpolicy:check`. For hook edits, run `pnpm hooks:test` and re-trust via `/hooks`.
