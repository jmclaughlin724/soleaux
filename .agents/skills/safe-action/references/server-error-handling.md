...
export const actionClient = createSafeActionClient({
  handleServerError: (error) => {
    ...
    // Supabase PostgREST/Auth errors are `{ data, error }` results, not
    // throws — catch them at the call site and rethrow a typed Error
    // (e.g. PortalActionError) so the discriminator below matches.
...
export const getPost = authActionClient
  .inputSchema(z.object({ postId: z.uuid() }))
