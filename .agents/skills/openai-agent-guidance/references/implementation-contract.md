...
- Put trusted-repository Codex settings in `.codex/config.toml`, lifecycle enforcement in hooks, Codex `prefix_rule` command execution policy in `.codex/rules`, and narrow subagent roles in `.codex/agents` or `~/.codex/agents`. A request merely described as a "rule" does not select `.codex/rules`.
